...
export class UserService {
    ...
    async addUserToGroup(dto : AddUserToGroupDto){
            ...
            await this.logsService.logAction(user , "Add-User" , "Group" , group.group_id , `Has Been Added to Group ${group.group_name}`)
    ...
    async getSummaryCharts (id : string) {
     ...
     async editUserInfo(
        ...
        const correctPassword = await bcrypt.compare(oldPassword , user.user_password);
        if(!correctPassword)
        ...
        if (newEmail && newEmail !== user.user_email) {
                const emailExists = await this.userRepo.findOne({ where : { user_email: newEmail }});
