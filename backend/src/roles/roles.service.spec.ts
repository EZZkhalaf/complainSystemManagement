...
describe('RolesService', () => {
  ...
  it('should throw new error when the permissions are not in an array ', async () => {
    ...
    await expect(
      service.addPermissions(user, invalidPermission),
    ).rejects.toThrow(
    ...
    await expect(
      service.addPermissions(user, invalidPermission2),
    ).rejects.toThrow(
      "Each permission must be an object with 'name' and 'description' as strings",
    );
  });
  it('should return a message when all the new permission are not added to the db bec they already exists ', async () => {
    ...
    expect(result.message).toEqual(
      'No new permissions to add (all already exist)',
    );
  ...
  it('should throw bad rewuest errror whe the role id is not valid o0t the permission and invalid too', async () => {
    ...
    await expect(service.addPermissionsToRole(dto, req)).rejects.toThrow(
      "Missing or invalid 'permissionsIds' array or 'roleId'",
    );
  });
  ...
  it('should throw BadRequestException if the role is admin or user', async () => {
    const req: any = { user: { user_id: 1 } };
    const id = '1';

    const fakeRole = {
      role_id: 1,
      role_name: 'user', // forbidden role
      users: [],
      permissions: [],
    };
  ...
  it('should delete role and move all its users to User role , save it and delete the role and then add log', async () => {
    const req: any = { user: { user_id: 1 } };
    const id = '1';
    const fakeUsers = [
      { user_id: 1, user_name: 'ezz', user_email: 'ezz@gmail.com' },
      { user_id: 2, user_name: 'ezz', user_email: 'ezz2@gmail.com' },
      { user_id: 3, user_name: 'ezz', user_email: 'ezz3@gmail.com' },
    ];
    ...
    rolesRepo.findOne
      .mockResolvedValueOnce(fakeRole) // first call -> roleToDelete
      .mockResolvedValueOnce(userRole); // second call -> userRole
    ...
    expect(logsService.logAction).toHaveBeenCalledWith(
      ...
      'Role',
      id,
      `Has Deleted the Role ${fakeRole.role_name}`, // should use deleted role's name
    );
  });
  ...
  it('should create a permission alone using the name and the description', async () => {
    ...
    const result = await service.createPermissionAlone(
      permissionName,
