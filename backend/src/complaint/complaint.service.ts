...
export class ComplaintService {
  ...
  async handleComplaintInGroup(id: string, dto: HandleComplaintInGroupDto) {
    ...
    if (!groups || !Array.isArray(groups))
      return {
        ...
        body: {
          success: false,
          message: 'Group sequence rule is missing or invalid.',
        },
      };
    ...
    if (complaint.groupsQueue.includes(currentGroup))
      return {
        ...
        body: {
          success: false,
          message: 'Current group already handled this complaint.',
        },
      };
    ...
    if (status === 'accept') {
      ...
      if (currentStep === groups.length - 1) {
        ...
        return {
          ...
          body: {
            success: true,
            message: 'Accepted and passed to next group.',
            complaint,
          },
        };
      }
    ...
    if (status === 'reject') {
      ...
      return {
        ...
        body: {
          success: true,
          message: 'Rejected and notified the user.',
          complaint,
        },
      };
    }
    ...
    return {
      ...
      body: {
        success: false,
        message: "Invalid status. Use 'accept' or 'reject' only.",
      },
    };
  }
  ...
  async getComplaintInfo(
    id: string,
  ): Promise<{ success: boolean; complaint: ComplaintOutputDto }> {
    ...
    return {
      success: true,
      complaint: plainToInstance(ComplaintOutputDto, complaint, {
        excludeExtraneousValues: true,
      }),
    };
  }
