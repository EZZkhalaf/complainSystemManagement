...
export class LeavesService {
  constructor(
    ...
    private readonly leavesRepo: Repository<LeavesEntity>,
  ...
  async changeLeaveState(leave_id: string, dto: ChangeLeaveStateDto) {
    ...
    await sendEmail(
      leave.leave_user.user_email,
      'Leave Update',
      `Your Leave has been ${new_state}`,
    );
  ...
  async deleteLeave(leave_id: string, user_id: string) {
    ...
    if (leave.leave_user.user_id !== user.user_id) {
      ...
      const is_member = hr_department.users.some(
  ...
  async getLeaves(dto: PaginAndFilterDto) {
    ...
    if (date_from && date_to)
      where.created_at = Between(new Date(date_from), new Date(date_to));
