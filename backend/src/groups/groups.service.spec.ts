...
describe('GroupsService', () => {
  ...
  it('should return the group and the users info in it and return success', async () => {
    ...
    const result = await service.getGroupInfoAndUsers(groupId);
    expect(result.success).toBe(true);
    const expectedResult = plainToInstance(GroupOutputDto, fakeGroup, {
  ...
  it('should return the users groups even if there none ', async () => {
    ...
    const expectedResult = plainToInstance(GroupOutputDto, fakeGroups, {
      excludeExtraneousValues: true,
    });
  ...
  it('should throw new not found error when the group not found', async () => {
    const groupId = '1';
    const dto: any = {
      userId: '1',
      type: 'general',
  ...
  it('should list all the group complaints and return success ', async () => {
    ...
    expect(complaintsRepo.createQueryBuilder).toHaveBeenCalledWith(
      'complaint_info',
    );
    expect(queryBuild.innerJoin).toHaveBeenCalledWith(
      'complaint_info.groupsQueue',
      'group_entity',
    );
    expect(queryBuild.where).toHaveBeenCalledWith(
      'group_entity.group_id = :groupId',
  ...
  it('should return the searched groups to the user ,  even if there is none', async () => {
    ...
    expect(queryBuild.where).toHaveBeenCalledWith(
      'group_entity.group_name ILIKE :search',
      { search: `%${search}%` },
    );
  });
  ...
  it('should throw error when the group is already in the rule ', async () => {
    ...
    expect(result.message).toEqual(
      'group already in the complaint groups sequence',
    );
